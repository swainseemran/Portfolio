...
const NavBar = () => {
  ...
  return (
    ...
    >
      <div>
        <h1 className="text-5xl font-signature ml-2">Seemran</h1>
      </div>
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="px-4 cursor-pointer captilize font-medium text-gray-500 hover:scale-105 duration-200"
          >
      ...
      bg-gradient-to-b from-black to-gray-800 text-gray-500"
        >
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer captilize py-6 text-4xl"
            >
